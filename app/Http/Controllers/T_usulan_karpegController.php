...
class T_usulan_karpegController extends Controller
{
    public function data_usulan_karpeg($id)
    {
        ...
        return DataTables::of($model)
            ->addColumn('action', function ($model) {
                return view('layouts._action2', [
                    'model' => $model,
                    'model2' => "Daftar Usulan Karpeg",
                    'url_edit' => route('usulan_karpeg.edit', $model->id_usulan_pegawai),
                    'url_destroy' => route('usulan_karpeg.destroy', $model->id_usulan_pegawai)
                ]);
            })
            ->addIndexColumn()
            ->rawColumns(['action'])
    ...
    public function show($id)
    {
        ...
        return view('t_views.t_usulan_pegawai.daftar_layanan_usulan.layanan_karpeg.form', compact('model', 'pegawai','model2'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('t_views.t_usulan_pegawai.daftar_layanan_usulan.layanan_karpeg.form', compact('model', 'pegawai'));
    }
    ...
    public function index_daftar_usulan_karpeg()
    {
        ...
        return view('t_views.t_usulan_pegawai.daftar_layanan_usulan.layanan_karpeg.daftar_usulan_karpeg', compact('nama_skpd'));
    }



    public function daftar_usulan_karpeg($id)
    {
        if ($id == 0) {
            $model =  T_usulan_pegawai::select('*')
            ->join('t_usulans', 't_usulans.id_usulan', '=', 't_usulan_pegawais.id_usulan')
            ...
            ->where('t_usulans.kategori_nomor_usulan','=','Usulan Karpeg')
            ->get();
        } else {
        ...
        return DataTables::of($model)
            ->addColumn('action', function ($model) {
                return view('layouts._action5', [
                    'model2' => "Verifikasi Berkas Usulan Karpeg",
                    'url_edit' => route('data_usulan_karpeg.verifikasi', $model->id_usulan_pegawai)
                ]);
            })
            ->addIndexColumn()
            ->rawColumns(['action'])
    ...
    public function verifikasi_daftar_usulan($id)
    {
        ...
        return view('t_views.t_usulan_pegawai.daftar_layanan_usulan.layanan_karpeg.form_daftar_usulan_karpeg', compact('nama_subbidang','status_berkas','status_proses','model', 'pegawai','data_prajabatan','jenis_jabatan'));
    }
