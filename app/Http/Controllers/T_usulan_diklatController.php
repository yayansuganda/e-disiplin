...
class T_usulan_diklatController extends Controller
{
    public function data_usulan_diklat($id)
    {
        ...
        return DataTables::of($model)
            ->addColumn('action', function ($model) {
                return view('layouts._action2', [
                    'model' => $model,
                    'model2' => "Daftar Usulan Diklat",
                    'url_edit' => route('usulan_diklat.edit', $model->id_usulan_pegawai),
                    'url_destroy' => route('usulan_diklat.destroy', $model->id_usulan_pegawai)
                ]);
            })
            ->addIndexColumn()
            ->rawColumns(['action'])
    ...
    public function show($id)
    {
        ...
        $jenis_diklat = DB::table("t_m_jenis_diklats")->get();


        $model = new T_nomor_usul();
        $model2 = T_nomor_usul::where('nomor_usulan', $id)->firstOrFail();

        return view('t_views.t_usulan_pegawai.daftar_layanan_usulan.layanan_diklat.form', compact('jenis_diklat','model', 'pegawai','model2'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('t_views.t_usulan_pegawai.daftar_layanan_usulan.layanan_diklat.form', compact('jenis_diklat','model', 'pegawai'));
    }
    ...
    public function index_daftar_usulan_diklat()
    {
        ...
        return view('t_views.t_usulan_pegawai.daftar_layanan_usulan.layanan_diklat.daftar_usulan_diklat', compact('nama_skpd'));
    }


    public function daftar_usulan_diklat($id)
    {
        if ($id == 0) {
            $model =  T_usulan_pegawai::select('*')
            ->join('t_usulans', 't_usulans.id_usulan', '=', 't_usulan_pegawais.id_usulan')
            ...
            ->where('t_usulans.kategori_nomor_usulan','=','Usulan Diklat')
            ->get();
        } else {
        ...
        return DataTables::of($model)
            ->addColumn('action', function ($model) {
                return view('layouts._action5', [
                    'model2' => "Verifikasi Berkas Usulan Diklat",
                    'url_edit' => route('data_usulan_diklat.verifikasi', $model->id_usulan_pegawai)
                ]);
            })
            ->addIndexColumn()
            ->rawColumns(['action'])
    ...
    public function verifikasi_daftar_usulan($id)
    {
        ...
        return view('t_views.t_usulan_pegawai.daftar_layanan_usulan.layanan_diklat.form_daftar_usulan_diklat', compact('nama_subbidang','status_berkas','status_proses','model', 'pegawai','data_prajabatan','jenis_jabatan','sk_pangkat_golongan_terakhir','sk_jabatan_terakhir'));
    }
