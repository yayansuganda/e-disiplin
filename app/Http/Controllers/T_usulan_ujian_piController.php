...
class T_usulan_ujian_piController extends Controller
{
    public function data_usulan_ujian_pi($id)
    {
        ...
        return DataTables::of($model)
            ->addColumn('action', function ($model) {
                return view('layouts._action2', [
                    'model' => $model,
                    'model2' => "Daftar Usulan Ujian PI",
                    'url_edit' => route('usulan_ujian_pi.edit', $model->id_usulan_pegawai),
                    'url_destroy' => route('usulan_ujian_pi.destroy', $model->id_usulan_pegawai)
                ]);
            })
            ->addIndexColumn()
            ->rawColumns(['action'])
    ...
    public function show($id)
    {
        ...
        return view('t_views.t_usulan_pegawai.daftar_layanan_usulan.layanan_ujian_pi.form', compact('model', 'pegawai','model2'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('t_views.t_usulan_pegawai.daftar_layanan_usulan.layanan_ujian_pi.form', compact('model', 'pegawai'));
    }
    ...
    public function index_daftar_usulan_ujian_pi()
    {
        ...
        return view('t_views.t_usulan_pegawai.daftar_layanan_usulan.layanan_ujian_pi.daftar_usulan_ujian_pi', compact('nama_skpd'));
    }


    public function daftar_usulan_ujian_pi($id)
    {
        if ($id == 0) {
            $model =  T_usulan_pegawai::select('*')
            ->join('t_usulans', 't_usulans.id_usulan', '=', 't_usulan_pegawais.id_usulan')
            ...
            ->where('t_usulans.kategori_nomor_usulan','=','Usulan Ujian Pi')
            ->get();
        } else {
        ...
        return DataTables::of($model)
            ->addColumn('action', function ($model) {
                return view('layouts._action5', [
                    'model2' => "Verifikasi Berkas Usulan Ujian PI",
                    'url_edit' => route('data_usulan_ujian_pi.verifikasi', $model->id_usulan_pegawai)
                ]);
            })
            ->addIndexColumn()
            ->rawColumns(['action'])
    ...
    public function verifikasi_daftar_usulan($id)
    {
        ...
        return view('t_views.t_usulan_pegawai.daftar_layanan_usulan.layanan_ujian_pi.form_daftar_usulan_ujian_pi', compact('nama_subbidang','status_berkas','status_proses','model', 'pegawai','data_prajabatan','jenis_jabatan','sk_pangkat_golongan_terakhir','sk_jabatan_terakhir'));
    }
    ...
    public function update_create_verifikasi(Request $request, $id)
    {
        if ($request->status_proses == "Selesai") {
            ...
            $this->validate($request, [
                ...
                'jenis_ujian_pi' => 'required',
            ]);

            $path_ujian_pi = $request->file('path_ujian_pi');
            $paths = $request->nip;

            $data = [
                        'nip' => $paths,
                        'jenis_ujian_pi' => $request->jenis_ujian_pi,
                        'nomor_sertifikat_ujian_pi' => $request->nomor_sertifikat_ujian_pi,
                        'tanggal_sertifikat_ujian_pi' => $request->tanggal_sertifikat_ujian_pi
                    ];


            if ($path_ujian_pi) {
                $nama_file_ujian_pi = 'SERTIFIKAT UJIAN PI '.$request->tanggal_sertifikat_ujian_pi.'.'.$path_ujian_pi->getClientOriginalExtension();
                $ujian_pi = Storage::putFileAs('public/'.$paths, $path_ujian_pi, $nama_file_ujian_pi);
                $data['nama_file_ujian_pi'] = $nama_file_ujian_pi;
                $data['path_ujian_pi'] = $ujian_pi;
            }

            T_ujian_pi::create($data);
