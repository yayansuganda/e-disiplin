{!! Form::model($model,  [
    'route'=>$model->exists ? ['update_data.usulan_diklat', $model->id_usulan_pegawai] : 'usulan_diklat.store',
...
<div class="row">
    <div class="col-md-10 offset-md-2">
            ...
            <div class="form-group row m-b-10">
                <label class="col-md-3 text-md-right col-form-label">Jenis Usulan Diklat</label>
                <div class="col-md-6">

                    <select id="lainnya" name="lainnya" class="default-select2 form-control">
                        <option label="---pilih---">
                        @foreach($jenis_diklat as $data_diklat)
                            <option value="{{$data_diklat->nama_m_jenis_diklat}}" {{$model->lainnya == $data_diklat->nama_m_jenis_diklat  ? 'selected' : ''}}>{{$data_diklat->nama_m_jenis_diklat}} </option>
                        @endforeach
